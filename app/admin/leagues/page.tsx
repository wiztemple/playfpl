...
interface TriggerCronSuccessResponse {
  ...
  totalRanksUpdated: number;
  errors: { leagueId: string; error: string }[];
}
...
interface AdminActionSuccessResponse {
  success: boolean;
  message: string;
  transactionId: string;
  status?: string;
...
export default function AdminLeaguesPage() {
  ...
  // Component State
  const [finalizingLeagueId, setFinalizingLeagueId] = useState<string | null>(
    null
  );
  ...
  const [withdrawalStatusFilter, setWithdrawalStatusFilter] =
    useState("REQUIRES_APPROVAL"); // Default filter
  const [processingWithdrawalId, setProcessingWithdrawalId] = useState<
    string | null
  >(null); // Tracks loading state for any withdrawal action
  const [showRejectModal, setShowRejectModal] =
    useState<ClientAdminWithdrawalRequest | null>(null); // Store tx data for modal
  ...
  } = useWithdrawalRequests(withdrawalStatusFilter);
  const {
    data: bankAccounts = [],
    isLoading: bankAccountsLoading,
    error: bankAccountsError,
  } = useUserBankAccounts(); // Use user's accounts for management view

  // --- Mutations ---

  // Mutation for triggering points/rank sync
  const triggerSyncMutation = useMutation<
    TriggerCronSuccessResponse,
    Error,
    void
  >({
    mutationFn: async () => {
      console.log(
        "[ADMIN_SYNC_MUTATION] Calling POST /api/admin/trigger-cron..."
      );
      ...
      if (!response.ok)
        throw new Error(
          data.error ||
            data.message ||
            `Sync trigger failed: ${response.statusText}`
        );
      return data;
    },
    onSuccess: (data) => {
      let description = `Processed ${
        data.processedLeagues || 0
      } active leagues. Updates: ${data.totalEntriesUpdated || 0} points, ${
        data.totalRanksUpdated || 0
      } ranks.`;
      if (data.errors?.length > 0) {
        description += ` Errors: ${data.errors.length}.`;
        toast.warning("Sync Completed with Errors", { description });
      } else {
        toast.success("Sync Triggered Successfully", { description });
      }
      queryClient.invalidateQueries({ queryKey: ["adminLeagues"] }); // Refresh league list potentially
    },
    onError: (error: Error) => {
      toast.error("Sync Trigger Failed", { description: error.message });
    },
  });

  // Mutation for finalizing a league
  const finalizeLeagueMutation = useMutation<
    FinalizeSuccessResponse,
    Error,
    string
  >({
    mutationFn: async (leagueId: string): Promise<FinalizeSuccessResponse> => {
      const response = await fetch(`/api/admin/leagues/${leagueId}/finalize`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      const data = await response.json();
      if (!response.ok)
        throw new Error(
          data.error ||
            data.details ||
            `Finalization failed: ${response.statusText}`
        );
      return { ...data, leagueId: leagueId };
    },
    onMutate: (leagueId: string) => {
      setFinalizingLeagueId(leagueId);
    },
    onSuccess: (data) => {
      toast.success(`League Finalized Successfully!`, {
        description: `League ${data.leagueId} marked as complete.`,
      });
      queryClient.invalidateQueries({ queryKey: ["adminLeagues"] });
      queryClient.invalidateQueries({ queryKey: ["league", data.leagueId] });
      queryClient.invalidateQueries({
        queryKey: ["leaderboard", data.leagueId],
      });
    },
    onError: (error: Error, leagueId) => {
      toast.error(`Failed to Finalize League ${leagueId}`, {
  ...
  const confirmDepositMutation = useMutation<
    ConfirmDepositSuccessResponse,
    Error,
    string
  >({
    mutationFn: async (
      transactionId: string
    ): Promise<ConfirmDepositSuccessResponse> => {
      ...
      if (!response.ok)
        throw new Error(
          data.error ||
            data.details ||
            `Confirmation failed: ${response.statusText}`
        );
      return data;
    },
    ...
    onSuccess: (data) => {
      toast.success(`Deposit Confirmed!`, {
        description: `Tx ID: ${data.transactionId} for user ${data.userId}.`,
      });
    ...
    onError: (error: Error, transactionId) => {
      toast.error(`Failed to Confirm Deposit ${transactionId}`, {
        description: error.message,
      });
    },
    onSettled: () => {
  ...
  const handleApproveWithdrawal = (txId: string) => {
    ...
    approveWithdrawalMutation.mutate(
      { transactionId: txId },
      { onSettled: () => setProcessingWithdrawalId(null) }
    );
  };
  const handleOpenRejectModal = (tx: ClientAdminWithdrawalRequest) => {
    setShowRejectModal(tx);
    setRejectReason("");
  };
  const handleConfirmRejectWithdrawal = () => {
    ...
    rejectWithdrawalMutation.mutate(
      {
        transactionId: showRejectModal.id,
  ...
  const handleOpenCompleteModal = (tx: ClientAdminWithdrawalRequest) => {
    setShowCompleteModal(tx);
    setBankReference("");
  };
  const handleConfirmCompleteWithdrawal = () => {
    ...
    completeWithdrawalMutation.mutate(
      {
        transactionId: showCompleteModal.id,
  ...
  const isLoading =
    authStatus === "loading" || leaguesLoading || depositsLoading;
    ...
    return (
      <div className="min-h-screen bg-gray-950 text-gray-100 flex items-center justify-center p-4">
        <Alert variant="destructive" className="max-w-lg">
          ...
          <AlertDescription>
            {(combinedError as Error)?.message || "Could not load admin data."}
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  return (
    <div className="min-h-screen max-w-6xl mx-auto bg-gray-950 text-gray-100 py-8 px-4 md:px-8 space-y-8">
      ...
      >
        <TabsList className="grid w-full grid-cols-5 bg-gray-800/60 border border-gray-700 mb-6">
          {" "}
          {/* Adjusted grid-cols */}
        ...
        <TabsContent value="leagues">
          ...
          <Card className="backdrop-blur-md bg-gray-900/60 border border-gray-800 shadow-lg">
            <CardHeader>
              ...
              <CardDescription>View and manage weekly leagues.</CardDescription>{" "}
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                      ...
                      <TableCell className="text-right space-x-2">
                        ...
                        {league.status === "completed" && (
                          ...
                          >
                            <Button
                              ...
                              variant="outline"
                              size="sm"
                              className="text-blue-300 border-blue-600/30 hover:bg-blue-950/30 hover:border-blue-500/50"
                            >
                              <a>
                          ...
                          league.isReadyToFinalize && (
                            <Button
                              ...
                              onClick={() => {
                                if (
                                  !(
                                    finalizingLeagueId === league.id ||
                                    finalizeLeagueMutation.isPending
                                  )
                                )
                                  finalizeLeagueMutation.mutate(league.id);
                              }}
                              disabled={
                                finalizingLeagueId === league.id ||
                                finalizeLeagueMutation.isPending
                        ...
                        {league.status === "active" &&
                          !league.isReadyToFinalize && (
                            <Badge
                              variant="outline"
                              className="text-xs text-amber-400 border-amber-600/30 bg-amber-950/20"
                            >
                              Awaiting FPL
                            </Badge>
                          )}
                        ...
                        {league.status === "upcoming" && (
                          <AdminLeagueActions
                            league={league as LeagueWithUserStatus}
                            isAdmin={true}
                          />
                        )}
                      </TableCell>
        ...
        <TabsContent value="deposits">
          ...
          <Card className="backdrop-blur-md bg-gray-900/60 border border-gray-800 shadow-lg">
            <CardHeader>
              ...
              <CardDescription>
                Review and confirm manual bank transfer deposits.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                    ...
                    >
                      ...
                      <TableCell className="text-right">
                        <Button
                          ...
                          onClick={() => {
                            if (
                              confirmingDepositId === deposit.id ||
        ...
        <TabsContent value="withdrawals">
          <Card className="backdrop-blur-md bg-gray-900/60 border border-gray-800 shadow-lg">
            <CardHeader>
              ...
              <CardDescription>
                Approve, reject, or complete withdrawal requests.
              </CardDescription>
              {/* Status Filter */}
              <div className="pt-4">
                ...
                <TableBody>
                  ...
                  {!withdrawalsLoading && withdrawalRequests.length === 0 && (
                    <TableRow>
                      ...
                      >
                        {" "}
                        No withdrawals found with status '
                  ...
                  {withdrawalRequests.map((tx) => {
                    ...
                    return (
                      <TableRow
                        key={tx.id}
                        className="border-gray-800 hover:bg-gray-800/50 text-sm"
                      >
                        ...
                        <TableCell className="text-xs text-gray-400">
                          ...
                          {!bankDetails?.bankName && (
                            <span className="italic">Details missing</span>
                          )}
                        </TableCell>
                        <TableCell>
                          <Badge
                        ...
                        <TableCell className="text-right space-x-1">
                          ...
                          {tx.status === "PROCESSING" && (
                            <Button
                              ...
                              onClick={() => handleOpenCompleteModal(tx)}
                              disabled={
                                isLoadingThisTx ||
        ...
        <TabsContent value="sync">
          ...
          <Card className="backdrop-blur-md bg-gray-900/60 border border-gray-800 shadow-lg">
            ...
            <CardContent>
              <Button
                onClick={() => {
                  if (!triggerSyncMutation.isPending)
              ...
              >
                {triggerSyncMutation.isPending ? (
                  ...
                  <>
                    <RefreshCw className="h-4 w-4 mr-2" /> Trigger Sync
                  </>
                )}
              </Button>
              {triggerSyncMutation.isError && (
                <p className="text-red-400 text-sm mt-3 p-3 bg-red-900/20 border border-red-800/50 rounded-md">
                  {" "}
                  <span className="font-medium">Sync Failed:</span>{" "}
                  {(triggerSyncMutation.error as Error)?.message ||
                    "Unknown error"}{" "}
                </p>
              )}
            </CardContent>
      ...
      <Dialog
        ...
        onOpenChange={(isOpen) => !isOpen && setShowRejectModal(null)}
      >
        <DialogContent className="sm:max-w-md bg-gray-900 border-gray-700">
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {" "}
              Tx ID: {showRejectModal?.id} <br /> User:{" "}
              {showRejectModal?.user?.name || showRejectModal?.user?.email}
              <br /> Amount:{" "}
              {formatCurrency(Number(showRejectModal?.amount || 0))}{" "}
            </DialogDescription>{" "}
          </DialogHeader>{" "}
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={processingWithdrawalId === showRejectModal?.id}
            >
              {" "}
              {processingWithdrawalId === showRejectModal?.id ? (
      ...
      <Dialog
        ...
        onOpenChange={(isOpen) => !isOpen && setShowCompleteModal(null)}
      >
        <DialogContent className="sm:max-w-md bg-gray-900 border-gray-700">
          {" "}
          <DialogHeader>
            ...
            <DialogDescription>
              {" "}
              Tx ID: {showCompleteModal?.id} <br /> User:{" "}
              {showCompleteModal?.user?.name || showCompleteModal?.user?.email}
              <br /> Amount:{" "}
              {formatCurrency(Number(showCompleteModal?.amount || 0))}{" "}
            </DialogDescription>{" "}
          </DialogHeader>{" "}
          <div className="py-4 space-y-2">
            {" "}
            <Label htmlFor="bankRef" className="text-gray-300">
            ...
            <p className="text-xs text-gray-500">
              Enter the reference from your bank transaction if available.
            </p>{" "}
          </div>{" "}
          <DialogFooter>
            {" "}
            ...
            <Button
              type="button"
              variant="success"
              onClick={handleConfirmCompleteWithdrawal}
              disabled={processingWithdrawalId === showCompleteModal?.id}
            >
              {" "}
              {processingWithdrawalId === showCompleteModal?.id ? (
                <Loader2 className="h-4 w-4 animate-spin mr-1" />
              ) : null}{" "}
              Mark as Completed{" "}
            </Button>{" "}
