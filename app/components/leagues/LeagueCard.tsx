...
export default function LeagueCard({
    ...
    const palettes = [
      {
        ...
        hovGrad: "hover:from-indigo-600 hover:to-purple-600",
      },
      {
        from: "from-blue-900/20",
        to: "to-cyan-900/10",
        accent: "bg-blue-600/20",
        accent2: "bg-cyan-600/10",
        grad: "from-blue-500 to-cyan-500",
        hovGrad: "hover:from-blue-600 hover:to-cyan-600",
      },
      {
        ...
        hovGrad: "hover:from-emerald-600 hover:to-teal-600",
      },
      {
        from: "from-amber-900/20",
        to: "to-orange-900/10",
        accent: "bg-amber-600/20",
        accent2: "bg-orange-600/10",
        grad: "from-amber-500 to-orange-500",
        hovGrad: "hover:from-amber-600 hover:to-orange-600",
      },
      {
        ...
        hovGrad: "hover:from-rose-600 hover:to-pink-600",
      },
      {
        from: "from-violet-900/20",
        to: "to-fuchsia-900/10",
        accent: "bg-violet-600/20",
        accent2: "bg-fuchsia-600/10",
        grad: "from-violet-500 to-fuchsia-500",
        hovGrad: "hover:from-violet-600 hover:to-fuchsia-600",
      },
    ];
  ...
  const getStatusConfig = (): StatusConfig => {
    switch (league.status) {
      case "upcoming":
        return {
          gradient: colorPalette.grad,
          hoverGradient: colorPalette.hovGrad,
          bgGlow: colorPalette.accent,
          label: "Upcoming",
        };
      case "active":
        return {
  ...
  return (
    ...
    >
      <Card className="w-full backdrop-blur-md bg-gray-900/60 border border-gray-800 shadow-xl hover:shadow-lg hover:shadow-indigo-500/20 hover:border-indigo-500/40 transition-all duration-300 relative overflow-hidden h-full flex flex-col">
        {/* Background effects */}
        <div
          className={`absolute inset-0 bg-gradient-to-br ${colorPalette.from} ${colorPalette.to} rounded-xl pointer-events-none opacity-70`}
        ></div>
        <div
          className={`absolute -right-16 -top-16 w-32 h-32 ${colorPalette.accent} rounded-full blur-3xl opacity-40`}
        ></div>
        <div
          className={`absolute -left-16 -bottom-16 w-32 h-32 ${colorPalette.accent2} rounded-full blur-3xl opacity-30`}
        ></div>
        ...
        <div
          className={cn(
            "absolute top-0 right-0 w-24 h-24 overflow-hidden z-10"
          )}
        >
          <div
            className={cn(
              "absolute top-0 right-0 transform rotate-45 translate-y-[20px] translate-x-[30px] w-28 text-center py-1 text-xs font-bold bg-gradient-to-r shadow-lg",
        ...
        <div className="p-4 md:p-5 relative z-10 flex flex-col flex-grow">
          ...
          <div className="flex justify-between items-center w-full">
            <div className="mb-3 flex items-center gap-2">
              <Badge
                variant="outline"
                className={cn(
                  "px-2 py-0.5 rounded-full text-xs font-semibold border-white/20",
                  colorPalette.grad,
                  "text-white"
                )}
              >
              ...
              <div>
                {mode === "available" && league.hasJoined && !isUpcoming && (
                  <div className="w-full text-center text-xs text-green-400 flex items-center justify-center">
                    <CheckCircle className="w-3 h-3 mr-1" /> Joined
                  </div>
                )}
              </div>
            </div>
            <div className="flex items-center text-xs border border-gray-700 p-1 rounded-2xl bg-indigo-600/20 from">
              <Calendar className="h-3.5 w-3.5 text-indigo-400 from-indigo-900/20 to-purple-900/20 mr-1" />
              GW {league.gameweek}
            </div>
          </div>
          ...
          {/* League Name */}
          <h3
            className="font-bold text-gray-100 text-lg mb-3 truncate drop-shadow-sm"
            title={league.name}
          >
            {league.name}
          </h3>
          ...
          <div className="flex justify-between items-center mb-3 space-x-2">
            <div className="bg-gradient-to-br from-gray-800/70 to-gray-800/50 backdrop-blur-sm rounded-lg px-3 py-1.5 flex-1 border border-gray-700/40 shadow-sm text-center">
              <p className="text-[11px] text-gray-300 flex items-center justify-center truncate">
                <PoolIcon className="h-3 w-3 mr-1 text-green-400 flex-shrink-0" />
                Prize Pool
              </p>
            ...
            <div className="bg-gradient-to-br from-gray-800/70 to-gray-800/50 backdrop-blur-sm rounded-lg px-3 py-1.5 flex-1 border border-gray-700/40 shadow-sm text-center">
              <p className="text-[11px] text-gray-300 flex items-center justify-center truncate">
                <EntryIcon className="h-3 w-3 mr-1 text-purple-400 flex-shrink-0" />
                Entry Fee
              </p>
            ...
            <div className="bg-gradient-to-br from-gray-800/70 to-gray-800/50 backdrop-blur-sm rounded-lg px-3 py-1.5 space-y-0.5 flex-1 border border-gray-700/40 text-center">
              <p className="text-[11px] text-gray-300 flex items-center justify-center truncate">
                <Clock className="h-3 w-3 mr-1 text-red-400 flex-shrink-0" />
                End Date
              </p>
              <p className="text-sm font-semibold text-red-400 truncate">
                {safeFormatDate(league.endDate)}
              </p>
            </div>
          </div>
          {/* Progress Bar */}
          <div className="flex items-center gap-3 mb-2">
            <div className="w-4/6 bg-gray-800/70 rounded-full h-1.5 overflow-hidden">
              <div
                className={cn(
                  "bg-gradient-to-r h-1.5 rounded-full",
                  statusConfig.gradient
                )}
                style={{
                  width: `${
            ...
            <div className="flex items-center text-xs w-fit border rounded-full px-2.5 py-1">
              <Users className="h-3.5 w-3.5 inline text-purple-400 mr-1" />
              <span>
                {league.currentParticipants ?? "?"}/
                {league.maxParticipants ?? "?"} Players
              </span>
            </div>
          </div>
          {mode === "joined" && isCompleted && positionConfig && (
            <div
              className={cn(
                "flex items-center justify-center mb-4 py-1.5 px-3 rounded-lg border backdrop-blur-sm shadow-sm",
                positionConfig.bgColor,
                positionConfig.borderColor
              )}
            >
              {positionConfig.icon}
              <span
              ...
              {userWinnings > 0 && (
                <span className="ml-2 text-green-400 font-semibold flex items-center text-sm">
                  {" "}
                  <Banknote className="h-4 w-4 mr-1" />{" "}
                  {formatCurrency(userWinnings)}{" "}
                </span>
              )}
            </div>
          )}
          <div className="w-full mt-auto pt-4 border-t border-gray-800/90">
            {mode === "available" && isUpcoming && !league.hasJoined ? (
              <Button
                onClick={onJoin}
                className={cn(
                  "w-full text-white border-0 flex items-center justify-center bg-gradient-to-r",
                  colorPalette.grad,
                  colorPalette.hovGrad
                )}
              >
                {" "}
                Join Now{" "}
              ...
              <Button
                onClick={onView}
                variant="outline"
                className="w-full border-indigo-600/30 text-indigo-300 bg-indigo-950/20 hover:bg-indigo-950/50 hover:border-indigo-500/50"
              >
                {" "}
                View Details{" "}
              </Button>
